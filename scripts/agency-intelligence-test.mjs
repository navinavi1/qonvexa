...
const id1=createJobIdentity({source:'workprotocol',externalId:'job-42'});
const id2=createJobIdentity({source:'workprotocol',externalId:'job-42'});
assert.equal(id1.id,id2.id);
assert.equal(id1.idempotencyKey,'12:workprotocol|6:job-42');

const learning=buildLearningSnapshot([
  {source:'workprotocol',externalId:'a',status:'settled',budgetUsd:100,costUsd:4,skill:'research-worker',at:'2026-09-01T10:00:00Z'},
  {source:'workprotocol',externalId:'b',status:'failed',budgetUsd:80,costUsd:2,skill:'research-worker',at:'2026-09-01T11:00:00Z'},
  {source:'clawlancer',externalId:'c',status:'paid',budgetUsd:50,costUsd:1,skill:'translation',at:'2026-09-01T12:00:00Z'},
  {source:'agenthansa',externalId:'d',status:'delivered',budgetUsd:30,costUsd:1,skill:'copywriting',at:'2026-09-01T13:00:00Z'}
],[]);
assert.equal(learning.sampleSize,4);
assert.equal(learning.sources.workprotocol.acceptanceRate,.5);
assert.equal(learning.skills.translation.acceptanceRate,1);
// 'delivered' means submitted, not accepted/paid (e.g. AgentHansa Earn, where a human
...
assert.equal(learning.sources.agenthansa,undefined);
assert.equal(learning.outcomes.delivered,1);

const scored=scoreOpportunity({
  source:'workprotocol',externalId:'x',budgetUsd:100,feePercent:2,escrowed:true,deadline:'2026-09-04T12:00:00Z',
