...
for(const title of ['Swap 1 USDC','Send USDT','Join our community','Invite your friends','Buy a service'])await test(`unsupported earning action: ${title}`,async()=>{assert.equal(classifyOpportunity({source:'workprotocol',title},{llmEnabled:true,hasAppTool:true}).executable,false);});
const store=new AutonomOSStore(dir);let registry=new JobRegistry({store});const op={source:'agenthansa',externalId:'ghost',title:'Old competition',claimMode:'competitive_submission',budgetUsd:5000};
await test('first AgentHansa miss persists across restart',async()=>{registry.observe(op);registry.setState(op,'proposal');registry.reconcileCompetitiveFeed('agenthansa',{ok:true,authoritativeLive:true,liveIds:[],complete:true});registry=new JobRegistry({store});assert.equal(registry.get(op).status,'stale_check');registry.observe({...op,title:'Changed cached title'});assert.equal(registry.get(op).status,'stale_check');assert.equal(registry.summary().proposal,0);});
await test('partial/failed feeds cannot archive a listing',async()=>{registry.reconcileCompetitiveFeed('agenthansa',{ok:false,authoritativeLive:true,liveIds:[],complete:true});registry.reconcileCompetitiveFeed('agenthansa',{ok:true,authoritativeLive:true,liveIds:[],complete:false});assert.equal(registry.get(op).status,'stale_check');});
await test('confirmed absence archives; ordinary observe cannot resurrect',async()=>{registry.reconcileCompetitiveFeed('agenthansa',{ok:true,authoritativeLive:true,liveIds:[],complete:true});registry.observe({...op,title:'changed'});assert.equal(registry.get(op).status,'archived');});
await test('authoritative live presence reopens an archived listing',async()=>{registry.reconcileCompetitiveFeed('agenthansa',{ok:true,authoritativeLive:true,liveIds:['ghost']});assert.equal(registry.get(op).status,'new');assert.equal(registry.get(op).terminal,false);});
await test('owned and permanently rejected jobs survive feed reconciliation',async()=>{registry.setState(op,'claimed');registry.reconcileCompetitiveFeed('agenthansa',{ok:true,authoritativeLive:true,liveIds:[],complete:true});assert.equal(registry.get(op).status,'claimed');registry.markPermanent(op,{reason:'closed'});registry.reconcileCompetitiveFeed('agenthansa',{ok:true,authoritativeLive:true,liveIds:['ghost']});assert.equal(registry.get(op).status,'graveyard');});
