...
const searchRoot=fs.mkdtempSync(path.join(os.tmpdir(),'search-hardening-'));
try {
  ...
  const r = await freeWebSearch('query', { STORAGE_DIR:searchRoot,AUTONOMOS_FREE_SEARCH_MIN_GAP_MS:'0' });
  assert(r.ok);
  assert.equal(r.provider,'free_public_web');
  assert.equal(r.results[0].url,'https://example.org/');
  globalThis.fetch = async () => new Response('rate limited',{status:429});
  assert.equal((await freeWebSearch('query-2',{STORAGE_DIR:searchRoot,AUTONOMOS_FREE_SEARCH_MIN_GAP_MS:'0'})).ok,false);
} finally {
  globalThis.fetch = originalFetch;fs.rmSync(searchRoot,{recursive:true,force:true});
}
