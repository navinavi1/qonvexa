...
globalThis.fetch=async(url,options={})=>{
  ...
  if(req.method==='tools/list')return rpc(req.id,{tools:surface==='legacy'?
    ...
    [{name:'t2000_jobs'},{name:'t2000_job_claim'},{name:'t2000_job_batch_claim'},{name:'t2000_job_status'},{name:'t2000_job_deliver'}]
  });
  if(req.method==='tools/call'){
    ...
    if(name==='t2000_jobs'){
      if(surface==='current'&&args?.role==='buyer')return rpc(req.id,{structuredContent:{openings:[{openingId:'open-current-50',maxUsdc:50,briefPreview:'Current buyer opening',slaMinutes:1440}]}});
      return rpc(req.id,{structuredContent:{jobs:[{jobId:'service-100',priceUsdc:100,serviceName:'Technical Research Premium',status:'funded'}]}});
    }
...
try{
  ...
  surface='current';calls.length=0;
  const current=await discoverPublicSignals(common);
  const currentOpen=current.signals.find(x=>x.externalId==='open-current-50');
  assert.ok(currentOpen,'current t2000_jobs(role=buyer) openings must remain discoverable when t2000_job_board is absent');
  assert.equal(currentOpen.budgetUsd,50);
  const buyerCalls=calls.filter(x=>x.method==='tools/call'&&x.params?.name==='t2000_jobs'&&x.params?.arguments?.role==='buyer');
  assert.ok(buyerCalls.length>=1,'compatibility path must read t2000_jobs with role=buyer');
  assert.equal(calls.some(x=>x.params?.name==='t2000_job_board'),false,'synthetic compatibility alias must never call a nonexistent remote tool');
  console.log('t2000 connector test PASS (legacy + current MCP surfaces)');
