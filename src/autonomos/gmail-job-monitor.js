...
export class GmailJobMonitor{
  ...
  async executeAndDeliver(id,action){
    ...
    const budget=createJobBudget(spendLimit,{env:this.env,jobId:'email_'+id,onCost:amount=>this.actioner.recordCost('email_'+id,amount)});const llm=budget.llm(this.actioner.llm);const executionConfig={...config,availableSpendUsd:spendLimit,maxPaidProcurementUsd:spendLimit};
