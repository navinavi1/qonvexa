...
export class AdaptiveSkillAcquirer{
  ...
  async tick(){if(this.running||!enabled(this.env.AUTONOMOS_SKILL_ACQUISITION_MODE,'true'))return;this.running=true;try{
    await refreshCapabilities(this.env);const gaps=this.collectGaps(),state=readJson(this.file,{workflows:{},events:[]});let tested=0;
    const checked=gap=>Date.parse(state.workflows[gap]?.lastTestAt||0)||0;gaps.sort((a,b)=>checked(a)-checked(b));
