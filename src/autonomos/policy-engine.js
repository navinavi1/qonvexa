export const DEFAULT_AUTONOMOS_CONFIG = Object.freeze({
  ...
  survivalMode: true,
  ownerRevenuePercent: 50,
  agentTreasuryPercent: 50,
  completionReservePercentOfPayout: 15,
  completionReserveMultiplier: 0.5,
  noAbandonAcceptedJobs: true,
  emergencyFinishMode: true,
  skillAcquisitionMode: true,
  zeroSpendMode: false,
  earnedFundsOnly: true,
  seedSpendBudgetUsd: 3,
  allowExternalSpending: false,
  minMarginPercent: 20,
  // Backward-compatible accounting fields. In survival mode reserve = owner share,
  // growth + experiment = the agent-treasury share.
  reservePercent: 50,
  growthPercent: 35,
  experimentPercent: 15,
  heartbeatSeconds: 60,
  fastClaimPollSeconds: 15,
  maxChildren: 50,
  childSpawnConcurrencyThreshold: 3,
  childTtlMinutes: 180,
  maxPaidProcurementUsd: 10,
  maxApiCostPercentOfPayout: 60,
  maxJobsPerCycle: 10,
  maxConcurrentJobs: 6,
  platformGeneration: 8,
  earningProfileVersion: 16,
  autoClaimJobs: true,
...
export function normalizeConfig(raw = {}) {
  ...
  if (legacy && Number(raw.maxJobsPerCycle) === 2) cfg.maxJobsPerCycle = 10;
  ...
  // Profile 16 is the Survival Swarm migration. Only replace known v15 defaults;
  // deliberate custom limits survive the migration.
  if(previousProfile < 16){
  ...
  cfg.earningProfileVersion = 16;
  cfg.enabled = Boolean(cfg.enabled);
  cfg.killSwitch = Boolean(cfg.killSwitch);
  cfg.survivalMode = cfg.survivalMode !== false;
  cfg.noAbandonAcceptedJobs = cfg.noAbandonAcceptedJobs !== false;
  cfg.emergencyFinishMode = cfg.emergencyFinishMode !== false;
  cfg.skillAcquisitionMode = cfg.skillAcquisitionMode !== false;
  ...
  cfg.minMarginPercent = clampNumber(cfg.minMarginPercent, 0, 95, 20);

  cfg.ownerRevenuePercent=clampNumber(cfg.ownerRevenuePercent,0,100,50);
  cfg.agentTreasuryPercent=clampNumber(cfg.agentTreasuryPercent,0,100,50);
  const revenueSplit=cfg.ownerRevenuePercent+cfg.agentTreasuryPercent;
  if(revenueSplit<=0){cfg.ownerRevenuePercent=50;cfg.agentTreasuryPercent=50;}
  else if(Math.abs(revenueSplit-100)>0.0001){
    cfg.ownerRevenuePercent=100*cfg.ownerRevenuePercent/revenueSplit;
    cfg.agentTreasuryPercent=100-cfg.ownerRevenuePercent;
  }
  cfg.completionReservePercentOfPayout=clampNumber(cfg.completionReservePercentOfPayout,0,50,15);
  cfg.completionReserveMultiplier=clampNumber(cfg.completionReserveMultiplier,0,3,0.5);

  // Existing ledger/spend code uses reserve vs growth+experiment. Survival mode makes
  // those legacy fields an exact projection of the requested 50/50 split.
  if(cfg.survivalMode){
    cfg.reservePercent=cfg.ownerRevenuePercent;
    cfg.growthPercent=cfg.agentTreasuryPercent*0.70;
    cfg.experimentPercent=cfg.agentTreasuryPercent*0.30;
  }else{
  ...
  cfg.maxChildren = Math.round(clampNumber(cfg.maxChildren, 1, 100, 50));
  ...
  cfg.maxPaidProcurementUsd = clampNumber(cfg.maxPaidProcurementUsd, 0, 100000, 10);
  cfg.maxApiCostPercentOfPayout = clampNumber(cfg.maxApiCostPercentOfPayout, 0, 80, 60);
  cfg.maxJobsPerCycle = Math.round(clampNumber(cfg.maxJobsPerCycle, 1, 50, 10));
  cfg.maxConcurrentJobs = Math.round(clampNumber(cfg.maxConcurrentJobs, 1, 20, 6));
  ...
  cfg.treasuryAsset = ['USDC','USDT','ETH','BTC','SOL'].includes(String(cfg.treasuryAsset).toUpperCase()) ? String(cfg.treasuryAsset).toUpperCase() : 'USDC';
...
export function validateAction(action = {}, config = {}) {
  ...
  // Survival autonomy never means custody of the owner's secrets. Agents may route to
  // verified public addresses, but never request/export seed phrases or private keys.
  if (action.kind === 'wallet_export' || action.kind === 'private_key_access') return { allowed:false, reason:'secret_access_forbidden' };
