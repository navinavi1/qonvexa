...
const SUCCESS=/^(paid|settled|completed|submitted|submitted_email)$/i;
...
export class SkillLibraryWorker{
  ...
  refresh(){try{
    ...
    const stats={};const observe=(skill,status,source='unknown',error='')=>{const key=String(skill||'general-digital');const row=stats[key]||={skill:key,attempts:0,successes:0,failures:0,sources:{},lastError:''};row.attempts++;if(SUCCESS.test(String(status||'')))row.successes++;if(FAILURE.test(String(status||''))){row.failures++;if(error)row.lastError=String(error).slice(0,240);}row.sources[source]=(row.sources[source]||0)+1;};
    ...
    for(const [skill,tools] of Object.entries(FREE_SKILL_MATRIX))if(!skills[skill])skills[skill]={...(previous.skills?.[skill]||{}),skill,attempts:0,successes:0,failures:0,successRate:0,tools:[...tools],priority:'learn',updatedAt:new Date().toISOString()};
