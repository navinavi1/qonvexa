...
async function inspectMarket(seed,candidate,env){
  ...
  const out={ok:true,host,openapiUrl,humanGate:HUMAN_GATE.test(evidence),payoutSignal:MONEY.test(evidence),registration:null,jobs:null,claim:null,delivery:null};
  if(!spec)return out;
  ...
  for(const [p,ops] of paths){for(const [method,op] of Object.entries(ops||{})){if(!/^(get|post|put|patch)$/i.test(method))continue;const text=`${p} ${op?.operationId||''} ${op?.summary||''} ${op?.description||''}`;
    ...
    if(!out.jobs&&/get/i.test(method)&&WORK.test(text)&&!/history|mine|my jobs|applications/i.test(text))out.jobs=operation(p,method,op,spec);
...
async function tryRegister(analysis,candidate,env){
  ...
  const schema=op.requestSchema||{};const required=Array.isArray(schema.required)?schema.required:[];const properties=schema.properties||{};const body={};const safeValues={name:'AutonomOS',agent_name:'AutonomOS',display_name:'AutonomOS',description:'Autonomous AI digital-services agency',callback_url:String(env.SITE_URL||env.RENDER_EXTERNAL_URL||'https://qonvexa.co').replace(/\/$/,'')+'/health',callbackUrl:String(env.SITE_URL||env.RENDER_EXTERNAL_URL||'https://qonvexa.co').replace(/\/$/,'')+'/health',wallet_address:String(env.AUTONOMOS_OWNER_WALLET||''),walletAddress:String(env.AUTONOMOS_OWNER_WALLET||''),website:String(env.SITE_URL||env.RENDER_EXTERNAL_URL||'https://qonvexa.co'),base_price:30,basePrice:30};
