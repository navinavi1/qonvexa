...
const REQUIRES_APP=/\b(send (?:an )?email|create (?:a )?calendar event|update (?:the )?crm|update (?:a )?(?:google )?sheet|post to (?:slack|reddit|x|twitter|linkedin|discord|telegram)|publish (?:on|to) (?:reddit|x|twitter|linkedin|discord|telegram)|create (?:a )?jira|create (?:a )?linear issue|edit (?:a )?notion|upload to (?:google )?drive|connected app)\b/i;
const REQUIRES_PROCUREMENT=/\b(hire (?:a |an |the )?(?:service|agent|provider|peer)|purchase (?:a |an |the )?(?:service|api|listing)|buy (?:a |an |the )?(?:service|api|listing)|post[, ]+hire[, ]+settle|pay (?:a |an |the )?(?:service|agent|provider))\b/i;
...
const REQUIRES_HUMAN_IDENTITY=/\b(kyc|selfie|government id|passport verification|personal account|aged account|account with \d+\+? (?:followers|karma|connections)|use your (?:reddit|x|twitter|linkedin|facebook|instagram) account|human verification|captcha solving)\b/i;
