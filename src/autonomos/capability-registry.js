...
export async function refreshCapabilities(env=process.env,{force=false,fetchImpl=fetch,shellProbe=null}={}){
  const key=resourceRoot(env);if(pending.has(key))return pending.get(key);
  const task=(async()=>{
    ...
    if(env.COMPOSIO_API_KEY&&resourceAvailability('composio',env).allowed&&should(state.apps)){
    ...
    if(/^https:\/\//i.test(String(env.AUTONOMOS_DEPLOY_WEBHOOK_URL||''))&&should(state.deploy)){
