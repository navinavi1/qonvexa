...
export class AutonomOSStore {
  ...
  readNdjson(name,limit=500){const file=this.file(name);if(limit===0)return[];if(limit<0)return readNdjsonCached(file);let text='';try{const stat=fs.statSync(file);if(limit>0&&stat.size>2*1024*1024){const bytes=Math.min(stat.size,Math.max(8192,limit*2048));const fd=fs.openSync(file,'r');const buf=Buffer.alloc(bytes);fs.readSync(fd,buf,0,bytes,stat.size-bytes);fs.closeSync(fd);text=buf.toString('utf8');const firstNewline=text.indexOf('\n');if(firstNewline>=0)text=text.slice(firstNewline+1);}else text=fs.readFileSync(file,'utf8');}catch{return[];}const lines=text.split(/\r?\n/).filter(Boolean),slice=limit>0?lines.slice(-limit):lines,rows=[];for(const line of slice){try{rows.push(JSON.parse(line));}catch{}}return rows;}
