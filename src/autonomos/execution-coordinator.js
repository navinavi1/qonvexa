...
export async function coordinateExecution(op,env,run){
 ...
 let pool=pools.get(root);if(!pool){pool={active:new Set(),pending:new Set(),waiting:[]};pools.set(root,pool);}
 if(pool.pending.has(key)||pool.active.has(key))throw Error('job_already_executing');
 const pump=()=>{
   pool.waiting.sort((a,b)=>a.deadline-b.deadline||b.profit-a.profit||a.order-b.order);
   while(pool.active.size<capacity&&pool.waiting.length){const next=pool.waiting.shift();pool.pending.delete(next.key);pool.active.add(next.key);next.resolve();}
 };
 pool.pending.add(key);
 await new Promise(resolve=>{pool.waiting.push({key,resolve,deadline:Date.parse(op.deadline||op.deadlineAt||'9999-01-01'),profit:Number(op.expectedNetProfit||op.budgetUsd||0),order:Date.now()});pump();});
 ...
 finally{pool.active.delete(key);pump();}
}
