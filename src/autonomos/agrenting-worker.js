...
export class AgrentingWorker{
  ...
  async executeHiring(h,credential){
    ...
    const budget=createJobBudget(Math.max(0,spendCap),{env:this.env,jobId:'agrenting_'+h.id,onCost:amount=>this.recordCost(h.id,amount)});const llm=budget.llm(this.llm);const execConfig={...config,availableSpendUsd:spendCap,maxPaidProcurementUsd:0,allowExternalSpending:false};
