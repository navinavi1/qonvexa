...
export async function planJob(opportunity,{llm=null,env=process.env,abortSignal=null,memoryContext=''}={}){
  ...
  const planningSignal=abortSignal?AbortSignal.any([abortSignal,timeout]):timeout;
  ...
  try{
    ...
    if(env.AUTONOMOS_USE_OPENAI_AGENTS_SDK!=='false'&&directOpenAIKey&&!env.LITELLM_BASE_URL&&!env.AUTONOMOS_LLM_BASE_URL&&!llm.budgeted){
      try{
        ...
        const out=await run(agent,request,{maxTurns:3,signal:planningSignal});
    ...
    if(abortSignal?.aborted)throw new Error('job_cancelled_by_emergency_stop');
    if(planningSignal.aborted)return fallback();
    const result=await llm.complete({messages:[{role:'system',content:'Plan legitimate paid work. Return JSON only.'},{role:'user',content:request}],maxTokens:Number(env.AUTONOMOS_PLANNER_MAX_TOKENS||1500),signal:planningSignal,task:'planning'});
