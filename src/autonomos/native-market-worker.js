...
export class NativeMarketWorker{
 ...
 async tick(){
  ...
  try{
   ...
   await Promise.allSettled(pending.slice(0,10).map(async job=>{
    ...
    try{if(adapter.prepare)await adapter.prepare();await this.process(job,adapter,config);}catch(e){this.save(job.id,{reason:String(e.message).slice(0,220),nextRetryAt:new Date(Date.now()+1800000).toISOString()});}
   }));
 ...
 async process(job,adapter,config){
  ...
  if(!job.applicationId&&!assigned){
   ...
   const cap=classifyOpportunity({...op,budgetUsd:op.payoutUsd},this.a.capabilityContext());if(!cap.executable)throw Error('capability_recovery_required');
   ...
   this.save(job.id,{opportunity:op,status:result.ok?'applied':result.uncertain?'application_uncertain':'application_failed',applicationId:result.proof?.externalId||'',applied:result.ok,reason:result.error||result.failure?.type||'',nextRetryAt:new Date(Date.now()+(result.ok?900000:3600000)).toISOString()});return;
  }
  if(['paid','settled','released'].includes(String(raw.payment_status||raw.payout?.status||'').toLowerCase())){
    ...
    if(received>0&&op.payoutUsd>0&&received>=op.payoutUsd){this.save(job.id,{status:'paid',paidAt:now()});return;}
  ...
  const budget=createJobBudget(limit,{env:this.env,onCost:n=>this.a.recordCost(job.id,n)}),llm=budget.llm(this.a.llm),opportunity={...op,jobId:job.id,budgetUsd:op.payoutUsd,claimMode:'already_assigned',status:'active'};
  ...
  this.save(job.id,{status:result.ok?'submitted':result.uncertain?'delivery_uncertain':'delivery_failed',delivered:result.ok,submissionId:result.proof?.externalId||'',submittedAt:result.ok?now():'',reason:result.error||result.failure?.type||'',nextRetryAt:new Date(Date.now()+900000).toISOString()});
 }
}
