...
async function orchestrateJobCore(opportunity,{llm,execute,memory=null,env=process.env,abortSignal=null,onEvent=()=>{}}={}){
  ...
  try{
    ...
    const graph=new StateGraph(State)
      .addNode('planner',async state=>{const plan=await planJob(state.opportunity,{llm,env,abortSignal,memoryContext:memoryPack.context});onEvent('job_planned',{source:plan.source,steps:plan.steps?.length||0});return{plan};})
      .addNode('executor',async state=>{if(abortSignal?.aborted)throw new Error('job_cancelled_by_emergency_stop');const deliverable=await execute({...state.opportunity,__plan:state.plan,__memoryContext:memoryPack.context});return{deliverable};})
      .addNode('qa',async state=>{const qa=await evaluateDeliverable(state.opportunity,state.deliverable,{llm,abortSignal,env});onEvent('qa_evaluated',{ok:qa.ok,score:qa.score,mode:qa.mode});if(!qa.ok)throw new Error(`qa_failed:${qa.reasons.join(',').slice(0,180)}`);return{qa,deliverable:{...state.deliverable,evidence:{...(state.deliverable.evidence||{}),plan:state.plan,qa,memoryHits:memoryPack.hits}}};})
    ...
    if(/^qa_failed:|job_cancelled_by_emergency_stop/.test(String(error?.message||'')))throw error;
    onEvent('langgraph_fallback',{error:String(error?.message||error).slice(0,180)});
    const plan=await planJob(opportunity,{llm,env,abortSignal,memoryContext:memoryPack.context});onEvent('job_planned',{source:plan.source,steps:plan.steps?.length||0});
