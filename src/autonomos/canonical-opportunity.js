...
export function canonicalOpportunity(raw={}){
 ...
 const payout=Number(raw.payout?.amount??(typeof raw.payout==='number'?raw.payout:undefined)??raw.budgetUsd??raw.amountUsd??0);
...
export function eligibility(op,env=process.env,{phase='execution'}={}){
 ...
 if((op.expectedNetProfit===null||op.expectedNetProfit<=0)&&!(phase==='application'&&op.applicationCostUsd===0&&op.expectedNetProfit===null))reasons.push('PROFITABILITY_UNVERIFIED_OR_NEGATIVE');
