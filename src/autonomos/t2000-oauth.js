...
async function discoverOAuth({ mcpUrl, fetchFn }) {
  ...
  try {
    const probe = await fetchWithTimeout(fetchFn, mcpUrl, {
      method: 'POST',
      headers: { 'content-type': 'application/json', accept: 'application/json, text/event-stream', 'user-agent': 'AutonomOS/7.7' },
...
async function resolveClientRegistration({ discovery, storedClient, clientMetadata, clientMetadataUrl, fetchFn }) {
  ...
  if (as.registration_endpoint) {
    ...
    const response = await fetchWithTimeout(fetchFn, endpoint, {
      method:'POST',
      headers:{ 'content-type':'application/json', accept:'application/json', 'user-agent':'AutonomOS/7.7' },
...
async function exchangeToken({ endpoint, params, clientSecret = '', authMethod = 'none', fetchFn }) {
  ...
  const headers = { 'content-type':'application/x-www-form-urlencoded', accept:'application/json', 'user-agent':'AutonomOS/7.7' };
...
async function fetchJson(fetchFn, url) {
  const response = await fetchWithTimeout(fetchFn, url, { headers:{ accept:'application/json', 'user-agent':'AutonomOS/7.7', 'mcp-protocol-version':'2025-06-18' } });
  const body = await readJsonResponse(response);
