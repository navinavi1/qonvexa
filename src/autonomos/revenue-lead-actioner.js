...
export class RevenueLeadActioner extends SearchFirstLeadActioner{
  priority(lead){let score=super.priority(lead);if(lead?.directRouteHint)score+=120;if(DIRECT_APPLICATION_HINT.test(String(lead?.discoveredBy||'')))score+=80;return score;}
  ...
  async sendApplicationEmailOnce(args){
    ...
    if(Number(payout?.amountUsd||0)<floor&&!allowUnpriced){if(id)this.setAction(id,{status:'payout_unverified',reason:`priced payout below verification floor (${Number(payout?.amountUsd||0)} < ${floor})`,nextRetryAt:new Date(Date.now()+12*60*60_000).toISOString()});this.event('lead_application_suppressed',{id,host,reason:'payout_not_priced_above_floor',amountUsd:Number(payout?.amountUsd||0),floor});return;}
    const now=Date.now(),events=Array.isArray(this.state?.events)?this.state.events:[];
    const hourCap=Math.max(1,Math.min(100,Number(this.env.AUTONOMOS_EMAIL_APPLICATIONS_PER_HOUR||20))),dayCap=Math.max(hourCap,Math.min(500,Number(this.env.AUTONOMOS_EMAIL_APPLICATIONS_PER_DAY||120)));
    ...
    if(result.needsConnectedAccount||/connected.?account|auth|unauthor|forbidden/i.test(`${result.error||''} ${result.detail||''}`)){this.setAction(id,{status:'email_channel_unavailable',reason:String(result.detail||result.error||'gmail_not_connected').slice(0,240),nextRetryAt:new Date(Date.now()+60*60_000).toISOString()});this.event('email_channel_unavailable',{id,host,error:String(result.error||'gmail_not_connected').slice(0,120)});return;}
    this.setAction(id,{status:'application_uncertain',reason:`email send outcome uncertain: ${String(result.detail||result.error||'unknown').slice(0,220)}`,recipient:route.email,nextCheckAt:new Date(Date.now()+60*60_000).toISOString()});this.event('lead_email_application_uncertain',{id,host,error:String(result.error||'unknown').slice(0,120)});
  }
}
function isSpecificWorkListing(lead){
  const title=clean(lead?.title).toLowerCase(),snippet=clean(lead?.snippet).toLowerCase(),url=String(lead?.url||'');if(!title)return false;
