...
export async function recoverFreeCapability(gap,env=process.env,{sandboxSession=null,signal=null}={}){
  ...
  const task=(async()=>{
    ...
    if(key.startsWith('connected_app:')||key==='connected_app_gateway'){await refreshCapabilities(env,{force:true,only:'apps'});const c=unifiedCapabilityContext(env);return{ok:key.includes(':')?c.connectedApps.includes(key.split(':')[1]):c.hasAppTool,route:'verified_connected_account',reason:'account_access_required_if_absent'};}
    ...
    if(key==='github_pr'||key==='sandbox_shell'){await refreshCapabilities(env,{only:key==='github_pr'?'github':'shell'});const c=unifiedCapabilityContext(env);return{ok:key==='github_pr'?c.hasGithubPrTool:c.hasShellTool,route:key==='github_pr'?'github_contents_pr':'e2b_shell'};}
