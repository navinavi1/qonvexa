export function resolveLlmEndpoint(env=process.env,{task='general'}={}){
  ...
  const direct=explicit||(openaiKey?'https://api.openai.com/v1':'');
  const modelMap=parse(env.AUTONOMOS_MODEL_ROUTING_JSON,{});
  return{
    baseUrl:lite||direct,
    apiKey:String(env.LITELLM_API_KEY||env.AUTONOMOS_LLM_API_KEY||openaiKey||''),
    model:String(modelMap[task]||env.AUTONOMOS_LLM_MODEL||'gpt-5-mini'),
    gateway:lite?'litellm':direct?'openai_compatible':'none'
  };
}
