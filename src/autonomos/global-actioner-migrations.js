...
export function migrateGlobalActionerState({env=process.env,storageDir='',logger=console}={}){
  ...
  for(const action of Object.values(state?.actions||{})){
    ...
    // 6–12 hour retry timers would leave hundreds of good leads asleep after the fix.
    ...
    if(!procurementFalsePositive&&!designChromeFalsePositive&&!staleRouteHold&&!staleEmailUnavailable&&!safePreSendFailure)continue;
    ...
    action.nextCheckAt='';
    action.missingTools=[];
    const reason=procurementFalsePositive?'procurement_classifier_fix':designChromeFalsePositive?'job_content_classifier_fix':staleRouteHold?'route_discovery_fix':staleEmailUnavailable?'gmail_send_fix':'pre_send_retry';
