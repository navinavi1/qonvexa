...
function renderAutonomOS(){
  ...
  if(tDetails){
    ...
    tDetails.innerHTML=t.connected
      ? `<span><b>Passport:</b> ${esc(address||'connected')}</span><span><b>USDC:</b> ${Number.isFinite(usdc)?usdc.toFixed(2):'—'}</span><span><b>Open jobs:</b> ${Number(h.openCount||0)}</span><span><b>Eligible ≥ $${Number(h.openFloorUsd??a.config?.t2000MinOpenJobPayoutUsd??0.5).toFixed(2)}:</b> ${Number(h.eligibleOpenCount||0)}</span><span><b>Priority ≥ $${Number(a.config?.t2000PriorityOpenJobPayoutUsd??65).toFixed(0)}:</b> ${Number(h.priorityOpenCount||0)}</span><span><b>Premium ≥ $${Number(a.config?.t2000PremiumOpenJobPayoutUsd??100).toFixed(0)}:</b> ${Number(h.premiumOpenCount||0)}</span><span><b>Seller queue:</b> ${Number(h.sellerQueueCount||0)}</span>${t.expiresAt?`<span><b>Session:</b> reconnect by ${esc(formatDate(t.expiresAt))}</span>`:''}`
