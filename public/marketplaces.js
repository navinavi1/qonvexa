...
function ensureGlobalFeedPanel(){
  ...
  // The work-pipeline panel used to be deleted here, before admin.js had bound to it.
  // That took the 13 queue tabs, the job search, the job-detail dialog, the funnel and
  // the money block out of the DOM, so their renderers returned on a null lookup and
  // their listeners bound to nothing. This feed sits alongside it instead.
...
if(typeof document!=='undefined'){
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{ensureGlobalFeedPanel();loadGlobalFeed();}
  ,{once:true});
  else {ensureGlobalFeedPanel();loadGlobalFeed();}
}
// Mission Control refreshes itself from admin.js now; this timer only reloads the feed
// file, and pauses while the tab is hidden.
setInterval(()=>{if(typeof document!=='undefined'&&!document.hidden&&document.documentElement?.isConnected)loadGlobalFeed();},10000);
